...
public class SimplifiedPearlSpawner : MonoBehaviour
{
    ...
    private void SpawnObject()
    {
        ...
        for (int attempt = 0; attempt < 100; attempt++)
        {
            ...
            if (IsValidSpawnPosition(spawnPosition))
            {
                ...
                Instantiate(objectsToInstantiate[selectedIndex], spawnPosition, Quaternion.identity);
                return;
            }
        }
    ...
    {
        return pos.position + new Vector3(
    ...
    {
        return Vector3.Distance(position, lastSpawnPosition) >= minDistanceFromLastSpawn &&
    ...
    private int GetWeightedRandomIndex()
    {
        ...
        return 0; // Fallback, should not reach here if weights are positive and valid
    }
    ...
    private bool ValidateWeights()
    {
        foreach (float weight in spawnWeights)
        {
            if (weight < 0)
            {
                Debug.LogError("Spawn weights contain negative values.");
                return false;
            }
        }
        return true;
