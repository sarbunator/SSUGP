...
public class PearlSpawner : MonoBehaviour
{
    ...
    public float minDistanceFromPlayer = 5f; // Minimum distance from the player to spawn pearls
    public float minDistanceFromOtherPearls = 2f; // Minimum distance from other pearls to spawn a new one
        ...
        while (true) // Infinite loop to keep spawning pearls
        {
            ...
            if (currentPearlCount < maxPearls) // Only spawn if the current count is below the max limit
            {
                for (int i = 0; i < spawnPoints.Count; i++)
                {
                    ...
                    if (Vector2.Distance(spawnPoint.position, playerTransform.position) >= minDistanceFromPlayer)
                    {
